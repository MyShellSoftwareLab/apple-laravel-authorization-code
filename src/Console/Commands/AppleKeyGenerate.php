...
namespace AnimusCoop\AppleTokenAuth\Console\Commands;
...
class AppleKeyGenerate extends Command
{
     ...
     * @var string
     */
    protected $signature = 'apple:help {--refresh : Refresh client secret}';
    ...
    public function setup()
    {
        ...
        config([
            ...
            'services.apple.key' => trim($auth_key),
    ...
    private function generateClientSecret($refresh=true)
    {
        ...
        $exists = Storage::disk('local')->exists(config('services.apple.key'));

        if($exists){
            $privateKeyFile = Storage::disk('local')->get(config('services.apple.key'));
            try{
            ...
            $this->error(config('services.apple.key').' - '.'File not found in the local driver path('.config("filesystems.disks.local.root").')');
